...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        except TypeError as wrong_number:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        updated_expression = Pair(target.second, expressions.second)
        value_bound = do_lambda_form(updated_expression, env)
        env.define(target.first, value_bound)
        return target.first
        # END PROBLEM 10
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    while expressions.second != nil:
        evaluation_two = scheme_eval(expressions.first, env)
        if scheme_truep(evaluation_two):
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        evaluation = scheme_eval(current.second.first, env)
        new_formals = Pair(current.first, new_formals)
        new_vals = Pair(evaluation, new_vals)
        check_form(current, 2, 2)
        check_formals(new_formals)
        bindings = bindings.second
    return env.make_child_frame(new_formals, new_vals)
